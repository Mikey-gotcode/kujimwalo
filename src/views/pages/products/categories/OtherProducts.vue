...
const checkout = async () => {
  ...
  try {
    await axios.post(`${api.baseURL}/orders`, payload, {
...
const loadProducts = async () => {
  ...
  //if (!authStore.token) return router.push('/signin');
  try {
    const res = await axios.get(`${api.baseURL}/products`, { headers: { Authorization: `Bearer ${authStore.token}`, Accept: 'application/json' }, withCredentials: true });
